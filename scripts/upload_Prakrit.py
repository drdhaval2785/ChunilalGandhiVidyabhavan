...
def uploadToArchive3(metadata):
	identifier = metadata['identifier']
	flog = codecs.open('../logs/uploadLog3.txt', 'a', 'utf-8')
	if len(identifier) > 100:
		print('File name too long: ' + identifier)
		flog.write('File name too long: ' + identifier + '\n----------\n')
	else:
		accession = metadata['Accession_No']
		acc = re.sub('^P', '', accession)
		startMessage = accession+'#'+identifier+'\n'+'Started at '+str(datetime.datetime.now())
		print(startMessage)
		flog.write(startMessage+'\n')
		r = upload(identifier, {identifier+'.pdf': '../../ChunilalGandhiMSS/compressedPdfFiles/Prakrit_-'+acc+'.pdf'}, metadata=metadata)
		endMessage=str(r[0].status_code)+'\n'+'Ended at '+str(datetime.datetime.now())+'\n----------\n'
		print(endMessage)
		flog.write(endMessage)
		flog.close()
	
def createMetadataJson3():
	fin = codecs.open('../derivedFiles/Prakritv001.tsv', 'r', 'utf-8')
	ferror = codecs.open('../logs/error3.txt', 'a', 'utf-8')
	print('Files not found')
	for line in fin:
		metadata = find_metadata2(line)
		identifier = metadata['identifier']
		accession = metadata['Accession_No']
		fullaccession = accession.replace('P','Prakrit_-')
...
if __name__=="__main__":
	...
	accessionsToBeUploaded = '../derivedFiles/uploadstack3.txt'
